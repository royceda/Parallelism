...
{
  // CPU side
  input_data = malloc(SIZE * SIZE * sizeof(float));
  output_data = malloc(SIZE * SIZE * sizeof(float));

  srand(1234);
  for(int y = 0; y < SIZE; y++)
    for(int x = 0; x < SIZE; x++)
      ((float (*)[SIZE])input_data)[y][x] = rand();

  // Allocate buffers inside device memory
  //
  input_buffer = clCreateBuffer(context,  CL_MEM_READ_WRITE,  sizeof(float) * SIZE * SIZE, NULL, NULL);
  if (!input_buffer)
    error("Failed to allocate input buffer");

  output_buffer = clCreateBuffer(context,  CL_MEM_READ_WRITE,  sizeof(float) * SIZE * SIZE, NULL, NULL);
  if (!output_buffer)
...
static void check_output_data(void)
{
  ...
  for(int y = 0; y < SIZE; y++)
    for(int x = 0; x < SIZE; x++) {
      float expected = ((float (*)[SIZE])input_data)[x][y];

    if(((float (*)[SIZE])output_data)[y][x] == expected)
      correct++;
  }
...
static void send_input(cl_command_queue queue)
{
  ...
  err = clEnqueueWriteBuffer(queue, input_buffer, CL_TRUE, 0,
			     sizeof(float) * SIZE * SIZE, input_data, 0, NULL, NULL);
...
static void retrieve_output(cl_command_queue queue)
{
  ...
  err = clEnqueueReadBuffer(queue, output_buffer, CL_TRUE, 0,
			    sizeof(float) * SIZE * SIZE, output_data, 0, NULL, NULL );  
