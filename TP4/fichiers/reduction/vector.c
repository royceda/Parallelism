...
{
  // CPU side
  input_data = malloc(2*SIZE * sizeof(float));
...
static void retrieve_output(cl_command_queue queue)
{
  ...
  check(err, "Failed to read input array");
}
...
int main(int argc, char** argv)
{
  ...
  while (argc > 1) {
	...
	THREADS = SIZE;
      ...
      r = sscanf(argv[1], "%u%[mMkK]", &THREADS, &c);
  ...
  for (cl_int p = 0; p < nb_platforms; p++) {
	...
	size_t global[1] = { THREADS };                      // global domain size for our calculation
	...
	{  
	  ...
	  err |= clSetKernelArg(kernel, 1, sizeof(int), &debut);
	  check(err, "Failed to set kernel arguments 2");
	  err |= clSetKernelArg(kernel, 2, sizeof(int), &fin);
	  ...
	  FIN = fin; // pour faire apparaitre le résultat
	}
